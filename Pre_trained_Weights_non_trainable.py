...
def index2sent2(x, desc_vocab): return indexesFromSentence(all_vocab, x, 1, 20)
    ...
    def _build_model(self):
        ...
        if self.text_embed_cols:
            ...
            for col, dim, seq_len in zip(self.text_embed_cols, 
                ...
                #x3 = GRU(32, return_sequences=True)(x3)
                #x3 = GRU(16, return_sequences=True)(x3)
                #x3 = Embedding(input_dim=dim[0], output_dim=dim[1], mask_zero=True)(x3)
...
if __name__ == "__main__":
    ...
    params = [55, 60, 7, 10, 17, 45, 252, 0.22586144757788901, 0.010774494286264107, 0.00062998520310506793]
    ...
    preds = cross_val_predict(nnet2, train_data, np.log1p(train_data.price), verbose=1, n_jobs=1)
    gc.collect()
